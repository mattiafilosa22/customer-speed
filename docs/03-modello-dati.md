...
- **`LeadStage.PRESENTATION_CALL_2` e `LeadStage.STANDBY`**: due nuovi stage pipeline (migrazione `20260714120000_pipeline_extras`, additiva — solo `ADD VALUE`, nessun rename/rimozione). `PRESENTATION_CALL_2` ("Seconda call") segue `PRESENTATION_CALL`; `STANDBY` ("Stand by") segue `WAITING_DECISION` e precede `WAITING_PAYMENT`. Label IT/EN nel layer i18n, colore default via i token CSS `--stage-presentation-2` / `--stage-standby` (`tokens.css`, mirrorati in `prisma/seed-helpers.ts` e `src/lib/theme.ts` per demo/preview coerenti). I nuovi seed/provisioning (`PIPELINE_STAGES` in `prisma/seed-helpers.ts`, `DEFAULT_PIPELINE_STAGES` in `src/server/admin/defaults.ts`) creano subito 11 righe `PipelineStageConfig` con `sortOrder` 0..10. **Tenant già esistenti** (righe `PipelineStageConfig` già persistite con la vecchia numerazione 0..8) NON vengono toccati da questa migrazione: `getPipelineConfig` sintetizza un `sortOrder` di visualizzazione per gli stage senza riga persistita interpolando tra i vicini canonici esistenti (mai un indice assoluto, per evitare collisioni con la vecchia numerazione), così i due nuovi stage compaiono subito nella posizione corretta nel kanban SENZA richiedere un riordino manuale — ma quel valore interpolato resta solo di visualizzazione (mai scritto su `PipelineStageConfig`) finché il tenant non salva un nuovo ordine da Impostazioni (`reorderStages`), che persiste `sortOrder` 0..10 pulito.
...
- **Insight & Stats** (feature flag `insightStats`, migrazione `20260910150541_insight_stats`, additiva): sezione che sostituisce il foglio "Sponsorizzate Instagram" (docs/superpowers/specs/2026-09-10-insight-stats-design.md). Aggiunge:
  - **enum `ChatChannel`** (`WELCOME` | `OUTBOUND_COMMENT` | `OUTBOUND_STORY` | `INBOUND`): canale della conversazione da cui nasce il lead. Commento e storia restano distinti sul lead ma condividono le colonne Appuntamenti/Vendite nella UI (raggruppati in `src/server/insight/channels.ts`).
  - **`Lead.chatChannel ChatChannel?`**: nullable in schema (i lead di altre provenienze non lo hanno), ma **obbligatorio in Zod** quando `Lead.sourceId` è la provenienza collegata alla sezione per il tenant (`Organization.insightSourceId`) — così ogni lead del report finisce in una colonna e non esiste un "secchio" di non attribuiti (i lead della provenienza collegata senza canale sono comunque possibili, es. dati storici/creati altrove, e alimentano l'avviso "non attribuiti" in pagina). Indice `@@index([organizationId, chatChannel, createdAt])`.
  - **`Lead.createdFromInsight Boolean @default(false)`**: true quando il lead nasce da un clic su una cella della sezione. Alimenta il tooltip "N creati da qui, M inseriti da lista/pipeline" senza dipendere dalla retention di `AuditLog`.
  - **`StageHistory` indice `[organizationId, toStage, changedAt]`**: le vendite si contano cercando i passaggi a `WON` in un intervallo; l'indice esistente `[organizationId, changedAt]` non ha lo stage nel prefisso.
  - **`Organization.insightSourceId String?`** (FK a `LeadSource`, `onDelete: SetNull`) e **`Organization.insightActiveFrom DateTime? @db.Date`**: la provenienza collegata alla sezione (non si cabla "Instagram": è configurabile per tenant) e il confine fra archivio importato (giorni `< insightActiveFrom`) e periodo vivo calcolato dalle entità.
  - **`ChatActivityDay`**: una riga per giorno di calendario per tenant con i sei contatori manuali (`welcomeSent`, `welcomeReplies`, `outboundComments`, `outboundStories`, `outboundReplies`, `inboundReceived`) che il CRM non può dedurre. Per il **solo periodo di archivio** (`isArchived: true`) conserva anche i conteggi congelati importati dall'Excel (`archivedOutboundMessages` e gli `archived*Appointments`/`archived*Sales` per canale): quei giorni non hanno entità dietro, quindi non sono ricalcolabili né cliccabili in UI. Appuntamenti e vendite del periodo **vivo** non stanno qui: si aggregano a ogni lettura da `Lead`/`Appointment`/`StageHistory` (mai materializzati). Vincolo `@@unique([organizationId, date])`, chiave dell'upsert usato sia dal salvataggio inline sia dall'import storico (idempotenza).
  - **Import storico** one-off (`scripts/import-insight-history.ts`, `pnpm db:import-insight`): legge il foglio "Chat", ignora le righe `TOT`/tassi (ricalcola i totali dai dati), scarta singolarmente le celle non numeriche segnalandole, importa ma segnala le righe con risposte > messaggi, rifiuta `date >= ACTIVATION_DATE`. Sempre dry-run finché non si passa `CONFIRM=yes`. Report in `backups/insight-import-<timestamp>.txt`.
...
| Lead | `[organizationId, chatChannel, createdAt]` | Insight & Stats: lead del canale X creati nel periodo Y |
| StageHistory | `[organizationId, toStage, changedAt]` | Insight & Stats: vendite (passaggi a WON) del periodo |
| ChatActivityDay | `[organizationId, date]` (unique) | riga del giorno per tenant; range scan per la lettura del mese, upsert idempotente per salvataggio/import |
