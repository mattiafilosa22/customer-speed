...
- **Data retention**: policy di conservazione configurabile (`Organization.leadRetentionMonths`, `docs/03` §3.3.1). **Implementata**: Settings → "Conservazione dati" (capability `settings.tenant`) mostra il conteggio dei lead PERSI (stage `LOST`, `lossReasonId` valorizzato) più vecchi della soglia configurata. Il flusso è sempre backup-prima-di-cancellare: "Esporta backup" (capability `lead.exportData`) scarica un JSON con tutti i dati del lotto candidato; solo dopo, "Elimina definitivamente" (capability `lead.eraseData`) anonimizza quegli stessi lead (stesso meccanismo di `eraseLeadData` §6.5 — note/riferimenti cancellati, identità anonimizzata, fatture mantenute per obbligo contabile). Il binding export→purge è un guardrail UX lato client (bottone abilitato solo dopo il download nella stessa sessione), non un controllo di sicurezza: la barriera reale è la capability `lead.eraseData` (mai `baseUser`) più l'isolamento tenant-scoped di `eraseLeadData`, che risolve un id di un altro tenant a no-op. Un lotto è limitato a 1000 lead (i più vecchi prima); tenant con più candidati ripetono il ciclo esporta→elimina.
